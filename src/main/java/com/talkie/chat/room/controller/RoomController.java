...
public class RoomController {
    ...
    @ApiResponse(responseCode = "400", description = "초대 대상이 비어있거나 GROUP인데 roomName이 없음",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    @ApiResponse(responseCode = "404", description = "존재하지 않는 초대 대상 포함",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    @PostMapping
    ...
    @ApiResponse(responseCode = "404", description = "방을 찾을 수 없거나 방 멤버가 아님",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
